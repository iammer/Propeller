...
PUB setCommand(command)
  long[bufferAddress][126]:=command
...
writeData     mov _i, par
              ...
              'mov localCnt, cnt
              'add localCnt, delay
:writeLoop    rdlong longToWrite, readFrom
              or outa, csPin

              sub readFrom, stopAt wz, nr
              muxnz outa, dcPin
              if_nz jmp #:notEnd

              mov readFrom, bufferLocation
              testn longToWrite, #0 wz
              if_z jmp #:writeLoop

              wrlong zero, stopAt

:notEnd       andn outa, clockPin
              mov _i, #32
              andn outa, csPin
:writeBit     test longToWrite, #1 wz
              muxnz outa, dinPin
              or outa, clockPin
              shr longToWrite, #1
              sub _i,#1 wz
